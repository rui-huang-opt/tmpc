...
class SetBase(metaclass=abc.ABCMeta):
    ...
    # 闵可夫斯基和（或平移）
    @abc.abstractmethod
    def __add__(self, other: Union["SetBase", np.ndarray]) -> "SetBase": ...

    # 庞特里亚金差，即闵可夫斯基和的逆运算
    # 即若 p2 = p1 + p3，则 p3 = p2 - p1，只有当输入为一个点（数组）时该运算等价于 (-p1) + p2
    @abc.abstractmethod
    def __sub__(self, other: Union["SetBase", np.ndarray]) -> "SetBase": ...
    ...
    @abc.abstractmethod
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs) -> "SetBase": ...

    # 集合的放缩
    @abc.abstractmethod
    def __mul__(self, other: Union[int, float]) -> "SetBase": ...

    def __rmul__(self, other: Union[int, float]) -> "SetBase":
        return self.__mul__(other)

    def __truediv__(self, other: Union[int, float]) -> "SetBase":
...
def support_fun(eta: np.ndarray, s: SetBase) -> Union[int, float]:
    if eta.ndim != 1:
